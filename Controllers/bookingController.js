...
const confirmBooking = async (req, res) => {
  ...
  try {
    ...
    if (existingBookings.length > 0) {
      ...
      const conflicted = seats.filter(s => alreadyBooked.includes(s));

      return res.status(400).json({
        message: `Seats already booked: ${conflicted.join(", ")}`
      });
    }

    // ------------------ CHECK HELD SEATS (DATE BASED) ------------------
    const now = new Date();
    const held = await TempBooking.find({
      theater_id,
      movie_title,
    ...
    const heldConflict = seats.filter(s => heldSeats.includes(s));

    if (heldConflict.length > 0) {
      return res.status(400).json({
        message: `Seats currently held: ${heldConflict.join(", ")}`
      });
    }
